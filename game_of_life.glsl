...
const vec4 deadColor = vec4(0.0, 0.0, 0.0, 0.0);
...
bool is CellAlive(int x, int y){
    vec4 pixel = imageLoad(inputImage, ivec 2(x, y));
    return pixel.r > 0.5;
}
...
void main() {
    ...
    int liveneighbors = getLiveNeighbors(pos.x, pos.y);
    bool isAlive = isCellAlive(pos.x, pos,y);
    bool nextState = isAlive;
    if(isAlive && (liveNeighbors < 2 || liveNeigbors > 3)){
        nextState = false;
